...
1. 准备 查询字符串，进行 ```URL Encode``` 并进行升序排列：

```
GET api.6pan.cn/v3/system/sign?language=%E5%85%AB%E5%9B%BD%E8%AF%AD%E8%A8%80&long=yes&play=%E5%A4%8F%E5%A8%81%E5%A4%B7%E5%90%89%E4%BB%96
```
注意这里 play 和 其他参数调换了位置。最终发送的请求里面我们也建议将其置换位置。

2. 准备 ```CanonicalizedHeaders``` 和 ```content-md5```
